...
namespace Neptun
{
	public class TakeSubjectViewModel : BaseViewModel
	{
		...
		private void LoadCourses()
		{
			...
			Task.Run(() =>
			{
				var courseschedulelist = new List<Tuple<string, string, string>>();
				...
				foreach (var info in asdasd)
					courseschedulelist.Add(new Tuple<string, string, string>(info.ChildNodes[1].InnerText, ((info.ChildNodes[7].InnerText == "Hétfo" ? "Hétfő" : info.ChildNodes[7].InnerText) + Environment.NewLine + info.ChildNodes[8].InnerText), info.ChildNodes[2].InnerText));

				foreach (var course in Courses)
					if (String.IsNullOrEmpty(course.Schedule))
					{
						var a = courseschedulelist.Find(s => s.Item1 == course.CourseCode);
						course.Schedule = a.Item2;
						course.Teacher = a.Item3;
					}
			});
			...
			Task.Run(() =>
			{
				...
				lock (RestWebClient)
				{
					...
					if (loadmenu)
					{
						...
						#region Courses
						var header = html.GetElementbyId("Addsubject_course1_gridCourses_headerrow").ChildNodes.Where(s => s.Name == "th").ToList();
						var pairs = new Dictionary<string, int>();
						for (int i = 0; i < header.Count; ++i)
							try
							{
								pairs.Add(String.IsNullOrEmpty(header[i].InnerText) ? i.ToString() : header[i].InnerText, i);
							}
							catch (Exception e)
							{
								Logger.LogDebug(e, "Adding to Dictionary throw an error");
							}
						var list = html.GetElementbyId("Addsubject_course1_gridCourses_bodytable").ChildNodes.FindFirst("tbody");
						foreach (var a in list.ChildNodes)
						{
							...
							try
							{
								...
								course.CourseCode = b[pairs["Kurzus kódja"]].ChildNodes[0].InnerText;
								var tmpasd = b[1].ChildNodes[0].GetAttributeValue("onclick", "");
								course.ID = String.IsNullOrWhiteSpace(tmpasd) ? "Imnotsure" : tmpasd.Split('(')[1].Split(',')[0].Replace("\'", "");
								course.ToolTip = b[1].ChildNodes.Count == 2 ? b[1].ChildNodes[1].InnerText : null;
								course.Type = b[pairs["Kurzus típusa"]].ChildNodes[0].InnerText;
								course.GroupName = b[2].ChildNodes[0].InnerText + id;
								course.Limits = b[pairs["Fő/Várólista/Limit"]].ChildNodes[0].InnerText;
								{
									if (pairs.TryGetValue("Rangsorsorrend", out int asd))
										course.rangsor = b[asd].InnerText;
								}
								{
									if (pairs.TryGetValue("Órarend infó", out int asd))
										course.Schedule = b[asd].ChildNodes.Count > 0 ? b[asd].ChildNodes[0].GetDirectInnerText() : String.Empty;
								}
								course.isEnabled = !b[1].HasClass("link_disabled");
								//Debugger.Break();
								//TODO: órarendi infó, different views on the server just cuz why not, dependent on semester?
								course.Teacher = b[pairs["Oktatók"]].ChildNodes[0].InnerText;
								course.Language = b[pairs["Nyelv"]].GetDirectInnerText();
								course.Note = b[pairs["Megj."]].ChildNodes.Count > 1 ? b[pairs["Megj."]].ChildNodes[1].InnerText : b[pairs["Megj."]].InnerText;
								course.isSelected = b[b.Count - 1].ChildNodes.FindFirst("input").GetAttributeValue("checked", "") == "checked";
							}
							catch (Exception e)
							{
						...
						Task.Run(() =>
				{
					try
					{
						...
						foreach (var course in Courses)
							...
							{
								FrameworkDI.Logger.LogDebugSource("Something not in the response", exception: e);
							}
						ParentViewModel.HasCourses.Invoke();
					}
					catch (Exception e)
					{
		...
		public TakeSubjectViewModel(string id, SubjectViewModel viewmodel, TFViewModel tFView, bool loadmenu = true)
		{
			...
			if (ParentViewModel.TakeViewModel == null)
				ParentViewModel.TakeViewModel = this;
